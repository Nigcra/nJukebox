...
async function initializeApp() {
  ...
  // Try automatic Spotify connection. The token comes from the data server,
  // which also keeps it fresh - there is no refresh interval in the browser.
  ...
  // Beim Start laden, nicht erst beim Oeffnen des Admin-Panels: der Auto-DJ
  // holt Playlists auch, wenn niemand den Tab je aufgeschlagen hat.
  loadSpotifyMarket();
...
// Spotify state - the token lives in the spotify.js module and comes from the
// data server. The browser keeps no copy in localStorage or sessionStorage.
...
async function loadLocalIndex() {
  ...
  debugLog('DATA-API', `Loaded ${tracks.length} tracks from server`);
    ...
    const libraryTracks = tracks.map(toLibraryTrack);
...
// Maps one row of /api/tracks into the library format. The library views span
// both sources: a row the union contributed from spotify_tracks carries source,
// uri and image_url and has a negative id - there is no file behind it, so
// neither the stream nor the cover endpoint applies.
function toLibraryTrack(track) {
  const isSpotify = track.source === 'spotify';

  return {
    type: isSpotify ? 'spotify' : 'server',
    id: track.id,
    title: track.title,
    artist: track.artist,
    album: track.album,
    ...
    uri: isSpotify ? track.uri : undefined,
    spotify_uri: isSpotify ? track.uri : undefined,
    streamUrl: isSpotify ? null : musicAPI.getStreamURL(track.id),
    coverUrl: isSpotify ? (track.image_url || '') : musicAPI.getCoverURL(track.id),
    image: isSpotify ? (track.image_url || '') : undefined,
...
function addToLibrary(items, source) {
  items.forEach((item) => {
    ...
    if (source === 'local') {
      ...
      libItem = {
        // A row that came from spotify_tracks keeps its type and uri - the
        // click handler and js/audio.js route on them.
        type: item.type === 'spotify' ? 'spotify' : 'server',
        id: item.id,
        title: item.title,
        path: item.streamUrl,
        image: item.coverUrl || '',
...
function applyAZFilterToArtists(letter) {
  ...
  if (visibleContainers.length === 0) {
    if (!existingNoResults) {
      ...
      const i18n = (typeof window.i18nSystem !== 'undefined' && window.i18nSystem) ? window.i18nSystem : null;
      noResultsDiv.textContent = letter === 'all'
        ? (i18n ? i18n.t('ui.messages.noArtistsFound') : 'Keine Künstler gefunden')
        : (i18n ? i18n.t('ui.messages.noArtistsWithLetter').replace('{letter}', letter) : `Keine Künstler gefunden, die mit "${letter}" beginnen`);
  ...
  try {
    ...
    if (source === 'spotify' && track.id) {
      ...
      const trackData = {
        ...
        // Spotify keeps genres on the artist. The server looks the genre up
        // from this id and falls back to the name when a track reaches it
        // without one - not every path into the queue carries the full object.
...
async function playSpotifyTrack(uri){
  ...
  const accessToken = window.spotifyAccessToken;
...
function openSpotifyLoginPopup() {
  ...
  // No token travels between the windows: the popup hands code and verifier to
  // the data server and closes itself. Once it is gone, the main window simply
  // asks the server for a token.
  const popupChecker = setInterval(() => {
    if (!popup.closed) return;

    clearInterval(popupChecker);
    debugLog('SPOTIFY', 'Popup closed, checking the login on the server...');
    ...
    autoConnectSpotify().then(success => {
      if (success) {
        ...
        debugLog('SPOTIFY', 'No Spotify login on the server after the popup');
      }
    });
  }, 1000);
}
...
function updateSpotifyUI() {
  ...
  // Der Auto-Learning-Tab haengt an der Anmeldung. Ohne diesen Aufruf bliebe er
  // nach einem Login im laufenden Betrieb verborgen, bis das Admin-Panel einmal
  // geschlossen und wieder geoeffnet wird.
  if (typeof updateSpotifyAutoLearnVisibility === 'function') {
    updateSpotifyAutoLearnVisibility();
  }
...
async function searchSpotifyDirect(query) {
  ...
  try {
    const res = await fetch(withSpotifyMarket(`https://api.spotify.com/v1/search?type=track&limit=20&q=${encodeURIComponent(query)}`), {
...
async function initialize(){
  ...
  // Beendet die Anwendung. Der Knopf sitzt im selben Kasten wie der
  // Auto-DJ-Schalter, nur wenige Pixel darunter, und schloss das Fenster bisher
  // ohne Rueckfrage - auf einem Touch-Geraet ein Fehlgriff mit ganzer Wirkung.
  quitButton.addEventListener('click', () => {
    debugLog('ui', '[QUIT] Quit-Knopf geklickt');
    if (confirm('nJukebox beenden und das Fenster schließen?')) {
      window.close();
    }
  });
...
function isAnyMusicPlaying() {
  ...
  if (queue && queue.length > 0 && currentTrackIndex >= 0 && currentTrackIndex < queue.length) {
    ...
    if (currentTrack) {
      ...
      //
      // Die Erkennung ist absichtlich so breit wie in js/audio.js:245. Nicht
      // jeder Weg, auf dem ein Track in die Warteschlange kommt, setzt type;
      // aus einer Playlist kommt er teils nur mit uri. Fiel er hier durch,
      // landete er im Zweig fuer lokale Titel, dort ist der Audio-Player
      // pausiert - und Equalizer wie Visualizer blieben stehen.
      const isSpotifyTrack = currentTrack.type === 'spotify' ||
        currentTrack.uri || currentTrack.spotify_uri || currentTrack.isSpotify;
...
// Blendet Tab und Inhalt des Auto-Learnings aus, solange keine Spotify-Anmeldung
// besteht - ohne sie scheitert dort jeder Knopf.
//
// Einzige Stelle dafuer: die Methode gleichen Zwecks in js/admin_panel.js
// faellt hierauf zurueck. Vorher gab es zwei Fassungen, die einander
// abwechselnd ein- und ausgeblendet haetten.
//
// Geprueft wird, ob ueberhaupt ein Token vorliegt - nicht, ob die lokale Kopie
// noch gilt. Sie ist laut js/spotify.js "never the source of truth": der Server
// erneuert im Hintergrund, und eine abgelaufene Kopie bedeutet nur, dass die
// Seite sie eine Weile nicht abgeholt hat. Mit isSpotifyTokenValid() waere der
// Tab bei bestehender Anmeldung verschwunden. Fehlt die Anmeldung wirklich,
// antwortet der Token-Endpoint mit 401 und setzt die Kopie auf null.
function updateSpotifyAutoLearnVisibility() {
  const autoLearnSection = document.getElementById('spotifyAutoLearnAdminSection');
...
// year:1970-1979 genre:disco returned 0 hits, and genre:eurodance got the 90s
// list to exactly one. The ones below return more than 100 each. Measure before
...
// heisst: Spotify nimmt das Land des angemeldeten Kontos, also das bisherige
// Verhalten. Der Wert liegt in app.db, nicht im Browser - eine Kiosk-Anlage
// wird einmal eingestellt und nicht bei jedem Rechner neu.
//
// Was market kann und was nicht: es filtert nach VERFUEGBARKEIT, nicht nach
// Herkunft. Dieselbe Anfrage liefert mit DE, US und JP Titel fuer Titel dasselbe
// Ergebnis - was hierzulande lizenziert ist, steht auch dort. Deutsche Musik
// bekommt man ueber das Genre (german pop, schlager, neue deutsche welle,
// deutschrap), nicht ueber dieses Feld.
let spotifyMarket = '';

// Haengt den Markt an eine Spotify-URL an. Ein Titel, den es im gewaehlten Land
// nicht gibt, faellt damit aus dem Ergebnis - genau der Zweck.
function withSpotifyMarket(url) {
  if (!spotifyMarket) return url;
  return `${url}${url.includes('?') ? '&' : '?'}market=${encodeURIComponent(spotifyMarket)}`;
}

// Zieht die Titel aus einer Playlist-Antwort. Beim Playlist-Endpoint entfernt
// market keinen einzigen Titel - Spotify liefert alle und markiert unspielbare
// nur mit is_playable: false. Das Aussortieren passiert deshalb hier; ohne
// gewaehltes Land fehlt das Feld und nichts wird angefasst.
function playablePlaylistTracks(items) {
  return (items || [])
    .map(item => item.track)
    .filter(track => track && !(spotifyMarket && track.is_playable === false));
}

async function loadSpotifyMarket() {
  const select = document.getElementById('spotifyMarket');

  try {
    if (window.settingsAPI) {
      spotifyMarket = await window.settingsAPI.getSetting('admin', 'spotifyMarket', '') || '';
    }
  } catch (error) {
    debugLog('main', '[MARKET] Land konnte nicht geladen werden:', error);
  }

  if (select) select.value = spotifyMarket;
}

async function saveSpotifyMarket(value) {
  spotifyMarket = value || '';

  try {
    if (window.settingsAPI) {
      await window.settingsAPI.setSetting('admin', 'spotifyMarket', spotifyMarket, 'string');
    }
  } catch (error) {
    debugLog('main', '[MARKET] Land konnte nicht gespeichert werden:', error);
  }
}

// Was beim letzten Lernen herauskam, je Playlist-Id: { count, at }. Liegt in
// app.db, damit die Angabe einen Neustart uebersteht - die gelernten Playlists
// selbst tun das nicht, sie leben nur zur Laufzeit.
let autoLearnStats = {};

async function loadAutoLearnStats() {
  try {
    if (window.settingsAPI) {
      const raw = await window.settingsAPI.getSetting('admin', 'autoLearnStats', '');
      autoLearnStats = raw ? JSON.parse(raw) : {};
    }
  } catch (error) {
    debugLog('main', '[AUTO-LEARN] Statistik konnte nicht geladen werden:', error);
    autoLearnStats = {};
  }
...
async function rememberAutoLearnResult(playlistId, count) {
  autoLearnStats[playlistId] = { count, at: Date.now() };
  renderAutoLearnStats();

  try {
    if (window.settingsAPI) {
      await window.settingsAPI.setSetting('admin', 'autoLearnStats', JSON.stringify(autoLearnStats), 'string');
    }
  } catch (error) {
    debugLog('main', '[AUTO-LEARN] Statistik konnte nicht gespeichert werden:', error);
  }
}

// "vor 5 Min.", "vor 3 Std.", "vor 2 Tagen" - genauer muss es nicht sein.
...
// Schreibt "50 Titel · vor 2 Std." als zweite Zeile in jeden Knopf, der schon
// einmal gelaufen ist.
//
// In den Knopf, nicht daneben: die Knoepfe sind unmittelbare Kinder eines
// zweispaltigen Grids, ein Geschwister-Element bekaeme dort eine eigene Zelle
// und stuende neben dem Knopf statt darunter. Die Beschriftung wird beim ersten
// Durchlauf in dataset.label gesichert, sonst waere sie nach dem ersten
// Schreiben Teil dessen, was beim naechsten Mal ueberschrieben wird.
function renderAutoLearnStats() {
  PARTY_PLAYLISTS.forEach(playlist => {
    ...
    const stat = autoLearnStats[playlist.id];
    if (!stat) {
      button.textContent = button.dataset.label;
      return;
    }

    button.innerHTML = `${escapeHtml(button.dataset.label)}<span style="display: block; font-size: 0.72em; color: #888; font-weight: normal; margin-top: 2px;">${stat.count} Titel · ${formatAge(stat.at)}</span>`;
  });
}
...
// Richtungen gerufen wird - von hier und aus js/admin_panel.js; hiesse sie wie
// die dortige Methode, riefe die sich selbst auf.
//
// Das Flag am Element haelt die beiden Wege auseinander: ohne es haette ein
// Klick zwei Suchanfragen ausgeloest.
function bindAutoLearnButtons() {
  const bind = (id, handler) => {
    const element = document.getElementById(id);
    if (!element || element.dataset.autoLearnBound === 'true') return;
    element.dataset.autoLearnBound = 'true';
    element.addEventListener('click', handler);
  };

  PARTY_PLAYLISTS.forEach(playlist => {
    bind(playlist.button, () => learnPartyPlaylist(playlist));
  });

  const market = document.getElementById('spotifyMarket');
  if (market && market.dataset.autoLearnBound !== 'true') {
    market.dataset.autoLearnBound = 'true';
    market.addEventListener('change', event => saveSpotifyMarket(event.target.value));
    loadSpotifyMarket();
  }
  ...
  // Batch buttons
  bind('learnAllParty', learnAllPartyPlaylists);

  // Custom playlist buttons
  bind('addCustomPlaylist', addCustomPlaylist);
  bind('loadAllCustom', loadAllCustomPlaylists);
  bind('clearCustom', clearCustomPlaylists);

  // Personal buttons
  bind('learnMyPlaylists', learnUserPlaylists);
  bind('learnMyTopTracks', learnUserTopTracks);
  bind('learnMyLibrary', learnUserLibrary);
}
...
async function learnTopTracksFromSpotify() {
  ...
  try {
    ...
    const response = await fetch(withSpotifyMarket('https://api.spotify.com/v1/playlists/37i9dQZEVXbMDoHDwVN2tF/tracks?limit=50'), {
      headers: { 'Authorization': `Bearer ${spotifyAccessToken}` }
    });
...
async function learnPopularPlaylistsFromSpotify() {
  ...
  try {
    ...
    const response = await fetch(withSpotifyMarket('https://api.spotify.com/v1/search?q=genre:pop&type=playlist&limit=5'), {
      headers: { 'Authorization': `Bearer ${spotifyAccessToken}` }
    });
    ...
    for (const playlist of playlists.slice(0, 3)) { // Limit to 3 playlists
      try {
        const tracksResponse = await fetch(withSpotifyMarket(`https://api.spotify.com/v1/playlists/${playlist.id}/tracks?limit=20`), {
          headers: { 'Authorization': `Bearer ${spotifyAccessToken}` }
        });
        
        if (tracksResponse.ok) {
          const tracksData = await tracksResponse.json();
          const tracks = playablePlaylistTracks(tracksData.items);
...
async function learnGenreHitsFromSpotify() {
  ...
  try {
    ...
    for (const genre of genres) {
      try {
        const response = await fetch(withSpotifyMarket(`https://api.spotify.com/v1/search?type=track&limit=10&q=genre:${genre}`), {
          headers: { 'Authorization': `Bearer ${spotifyAccessToken}` }
        });
...
async function learnUserPlaylistsFromSpotify() {
  ...
  try {
    ...
    for (const playlist of playlists.slice(0, 5)) { // Limit to 5 playlists
      try {
        const tracksResponse = await fetch(withSpotifyMarket(`https://api.spotify.com/v1/playlists/${playlist.id}/tracks?limit=50`), {
          headers: { 'Authorization': `Bearer ${spotifyAccessToken}` }
        });
...
async function bulkAddSpotifyTracks(tracks, source = '') {
  ...
  // Most callers hand in raw Spotify objects, but some paths pass tracks that
  // went through a mapping first - there album is a plain string and the image
  // sits in image or spotifyAlbumImage. Reading both shapes keeps a title from
  // landing in the library as "Unknown Album" without a cover.
  const tracksData = tracks.map(track => ({
    spotify_id: track.id || track.spotifyId || '',
    name: track.name || track.title || '',
    artist: primarySpotifyArtist(track),
    album: track.album?.name || (typeof track.album === 'string' ? track.album : '') || 'Unknown Album',
    year: track.album?.release_date?.slice(0, 4) || track.year || '',
    genre: source || 'Unknown',
    duration_ms: track.duration_ms || 0,
    image_url: track.album?.images?.[1]?.url || track.album?.images?.[0]?.url || track.image || track.spotifyAlbumImage || '',
    preview_url: track.preview_url || '',
    spotify_uri: track.uri || track.spotifyUri || '',
    popularity: track.popularity || 0
  }));
...
async function learnTracks(fetchTracks, playlistId, name, category = 'general', button = null, emptyHint = 'die Suche trifft nichts.') {
  ...
  try {
    ...
    await rememberAutoLearnResult(playlistId, tracks.length);

    // Eine leere Antwort ist kein Fehler, aber auch kein Erfolg: sie bedeutet,
    ...
    // Erst nach dem Zuruecksetzen: originalText ist der Stand von vorhin und
...
async function learnAllPartyPlaylists() {
  ...
  try {
    // Genau die Liste, die auch hinter den einzelnen Knoepfen steht.
    let total = 0;
    const empty = [];
    const failed = [];

    for (let i = 0; i < PARTY_PLAYLISTS.length; i++) {
      if (i > 0) {
        await new Promise(resolve => setTimeout(resolve, 1000));
      }

      const playlist = PARTY_PLAYLISTS[i];

      // Der Fortschritt steht auf dem Sammelknopf. Die einzelnen Aufrufe
      // bekommen ihn deshalb nicht uebergeben - sonst haetten sie ihn mit
      // ihrem eigenen "Lade..." ueberschrieben.
      if (button) {
        button.innerHTML = `⏳ ${i + 1}/${PARTY_PLAYLISTS.length} — ${playlist.name}`;
      }

      // undefined heisst gescheitert, 0 heisst die Suche trifft nichts - zwei
      // verschiedene Dinge, die verschiedene Abhilfe brauchen.
      const count = await learnPartyPlaylist(playlist);
      if (count === undefined) {
        failed.push(playlist.name);
      } else if (count === 0) {
        empty.push(playlist.name);
      } else {
        total += count;
      }
    }
    ...
    const notes = [];
    if (empty.length > 0) notes.push(`ohne Treffer: ${empty.join(', ')}`);
    if (failed.length > 0) notes.push(`fehlgeschlagen: ${failed.join(', ')}`);

    if (notes.length > 0) {
      toast.info(`${total} Titel geladen — ${notes.join('; ')}`);
    } else {
      toast.success(`${total} Titel aus ${PARTY_PLAYLISTS.length} Playlists geladen!`);
    }
...
function renderCustomPlaylists() {
  ...
  // Die Id, nicht der Index: die Liste wird nach jedem Loeschen neu geladen, und
  // ein gemerkter Index zeigte danach auf den falschen Eintrag. Genau daran ist
  // "Laden" bisher gescheitert - der Aufruf uebergab die Id, die Funktion
  // erwartete einen Index und griff ins Leere.
  container.innerHTML = customPlaylists.map((playlist) => {
    const stat = autoLearnStats[`custom_${playlist.id}`];
    const note = stat ? `${stat.count} Titel · ${formatAge(stat.at)}` : (playlist.spotify_url || playlist.url);

    return `
    <div class="custom-playlist-item">
      <div class="custom-playlist-info">
        <div class="custom-playlist-name">${escapeHtml(playlist.name)}</div>
        <div class="custom-playlist-url">${escapeHtml(note)}</div>
      </div>
      <div class="custom-playlist-actions">
        <button class="custom-playlist-btn load" onclick="loadCustomPlaylistById(${playlist.id})">Laden</button>
...
// Holt Name und Titelzahl zu einer Playlist-URL. Ohne das musste beides von Hand
// eingetippt werden, obwohl Spotify es kennt.
async function fetchSpotifyPlaylistInfo(playlistId) {
  if (!spotifyAccessToken) return null;

  try {
    const response = await fetch(withSpotifyMarket(
    ...
    const data = await response.json();
    return { name: data.name || '', total: data.tracks?.total ?? null };
  } catch (error) {
    debugLog('main', '[CUSTOM PLAYLISTS] Playlist-Info nicht abrufbar:', error);
    return null;
  }
}

// Zieht die Playlist-Id aus einer Spotify-URL, egal ob mit Query-Anhang oder als
// spotify:playlist:-Uri eingefuegt.
...
async function addCustomPlaylist() {
  ...
  if (!url) {
    toast.error('Spotify-URL ist erforderlich');
    return;
  }
  ...
  // Der Name darf leer bleiben - Spotify kennt ihn. Nur wenn der Abruf nichts
  // hergibt (kein Token, private Playlist), muss der Benutzer selbst ran.
  let name = nameInput.value.trim();
  if (!name) {
    const info = await fetchSpotifyPlaylistInfo(playlistId);
    name = info?.name || '';
  }

  if (!name) {
    toast.error('Name konnte nicht von Spotify geholt werden - bitte eintragen');
    nameInput.focus();
    return;
  }
...
// Laedt eine gespeicherte Playlist anhand ihrer Datenbank-Id.
async function loadCustomPlaylistById(id) {
  ...
  try {
    // Der Server speichert die Adresse als spotify_url, aeltere Eintraege als
    // url - beide Formen kommen vor.
    const playlistId = extractSpotifyPlaylistId(playlist.spotify_url || playlist.url || '');
    if (!playlistId) {
    ...
    await bulkAddSpotifyTracks(tracks, playlist.name);
    await rememberAutoLearnResult(`custom_${playlist.id}`, tracks.length);
    renderCustomPlaylists();

    toast.success(`${tracks.length} Titel aus "${playlist.name}" hinzugefügt!`);
...
async function loadAllCustomPlaylists() {
  ...
  if (customPlaylists.length === 0) {
    toast.info('Keine gespeicherten Playlists');
    return;
  }
  ...
  try {
    ...
    for (let i = 0; i < customPlaylists.length; i++) {
      ...
      if (button) {
        button.innerHTML = `⏳ ${i + 1}/${customPlaylists.length} — ${playlist.name}`;
      }

      // Jede Liste laeuft ueber denselben Weg wie ihr eigener Laden-Knopf, also
      // samt Merken des Ergebnisses. Ein Fehlschlag stoppt die anderen nicht.
      const count = await loadCustomPlaylistById(playlist.id);
    ...
    if (failed.length > 0) {
      toast.info(`${totalTracks} Titel geladen. Fehlgeschlagen: ${failed.join(', ')}`);
    } else {
      toast.success(`${totalTracks} Titel aus ${customPlaylists.length} Playlists hinzugefügt!`);
    }
...
async function clearCustomPlaylists() {
  ...
  if (!confirm(`Alle ${customPlaylists.length} Custom Playlists löschen?\n\n${names}\n\nDie bereits übernommenen Titel bleiben in der Bibliothek.`)) return;
...
// Der Zustand wird auf window gespiegelt, weil js/audio.js und js/playlists.js
// ihn dort lesen. Mit let allein steht er nur im globalen Scope, nicht als
// window-Eigenschaft - window.isAutoDjActive war damit immer undefined, und die
// Abfrage in stopAllPlayback() lief ins Leere: der Stop-Knopf hat den Auto-DJ
// nie angehalten, er schob gleich den naechsten Titel nach.
let isAutoDjActive = false;
window.isAutoDjActive = false;
...
function updateAutoDjStatusDisplay(isActive) {
  ...
  // Der Schalter im Admin-Panel wurde bisher nur beim Laden der Einstellungen
  // gesetzt. Schaltete etwas anderes den Auto-DJ ab - stopAllPlayback() tut das
  // - blieb er auf "an" stehen, und der naechste Klick darauf startete den
  // Auto-DJ, statt ihn zu stoppen.
...
function startAutoDj() {
  ...
  autoDjPlaybackInterval = setInterval(() => {
    ...
    //
    // isAnyMusicPlaying() statt audioPlayer.paused: Spotify-Titel laufen ueber
    // das Web-Playback-SDK, das Audio-Element bleibt dabei pausiert. Der
    // Watchdog weiter unten hielt deshalb jeden Spotify-Titel fuer haengend
    // und uebersprang ihn nach der Karenzzeit - rund 23 Sekunden nach Start.
    const isPlaying = isAnyMusicPlaying();
