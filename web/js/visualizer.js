// visualizer.js
// Now playing visualizations: space, fire, particles and circles
// Version: 2026.08.16
//
// Restored from the state before commit b8b505d ("Remove lightning vis") in the
// original project. That commit rewrote this file from 1226 lines down to 676
// and replaced every mode with a stripped variant, dropped the cover based
// background, and pointed the canvas lookup at a class that exists nowhere - so
// nothing was drawn at all. What is kept from it is its purpose: the lightning
// mode is gone.
...
(function() {
  ...
  function startVisualizerAnimation(isMusicPlayingCallback) {
    ...
    // Every mode advances by a fixed amount per frame - particle positions and
    // damping, star speed, life, the energy decay, starPulseTime. All of those
    // constants assume the 60 Hz that requestAnimationFrame delivered when they
    // were written, so on a 165 Hz display the whole visualization runs 2.75x
    // too fast. Skipping surplus frames keeps every constant at its designed
    // rate; rescaling them one by one would not.
    const TARGET_FRAME_MS = 1000 / 60;
    let lastFrameTime = -Infinity;

    function drawVisualizer(frameTime) {
      // The tolerance keeps a display running at exactly 60 Hz from dropping to
      // 30 by missing the threshold on timing jitter.
      const now = typeof frameTime === 'number' ? frameTime : performance.now();
      if (now - lastFrameTime < TARGET_FRAME_MS - 1) {
