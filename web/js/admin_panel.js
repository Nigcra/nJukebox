...
class AdminSettingsManager {
  ...
  //
  // Diese Methode hatte eine eigene, unvollstaendige Verdrahtung: sie kannte
  // drei Knoepfe, die es im Markup nicht mehr gibt, und zwei, die daneben schon
  // in jukebox.js haengen - ein Klick auf "Schlager Party" hat damit zwei
  // Suchanfragen ausgeloest und zwei verschieden benannte Playlists angelegt.
  // Die Liste steht jetzt an einer Stelle, in PARTY_PLAYLISTS.
  initializeAdminAutoLearnButtons() {
    if (typeof window.bindAutoLearnButtons === 'function') {
      window.bindAutoLearnButtons();
    }
  ...
  //
  // Diese Methode hatte eine eigene Fassung derselben Logik - mit der Id
  // autoLearnSection, die es im Markup nicht gibt, und einer schwaecheren
  // Pruefung: sie sah nur nach, ob ueberhaupt ein Token da ist, nicht ob es
  // noch gilt. Bei abgelaufenem Token haetten sich die beiden Fassungen
  // gegenseitig ueberschrieben. Die Logik steht jetzt nur noch in jukebox.js.
  updateAdminAutoLearnStatus() {
    if (typeof window.updateSpotifyAutoLearnVisibility === 'function') {
      window.updateSpotifyAutoLearnVisibility();
    }
  }
